...
@router.get("/")
async def get_all_terminologies(client: Annotated[WeaviateRepository, Depends(get_client)]):
    terminologies = client.get_all_terminologies()
    return terminologies


@router.put("/{id}")
async def create_terminology(id: str, name: str, client: Annotated[WeaviateRepository, Depends(get_client)]):
    try:
